...
import UserProfileModal from '../AccountComps/UserProfileModal';
...
const Account = () => {
  ...
  return (
    <div className="relative p-4 min-h-screen w-full bg-gradient-to-b from-red-100 to-slate-400">
      ...
      <div className='flex flex-col md:flex-row'>
        <div className="md:w-6/12 mb-8 md:mb-0">
          ...
          <h1 className="text-2xl font-bold mb-4 text-center">
              ¡Te damos la bienvenida! {userData?.name || 'Usuario'}
            </h1>
          </div>
          ...
          <button onClick={() => setIsProfileModalOpen(true)} className="block text-blue-600 hover:underline">Mi cuenta</button>
                <button onClick={() => setIsPurchaseHistoryModalOpen(true)} className="block hover:underline">Todas las compras</button>
            <LinkRouter to="/Accounts/invite" className="block hover:underline">Invitar a un amigo</LinkRouter>
            <LinkRouter to="/Accounts/points-history" className="block hover:underline">Mi historial de puntos</LinkRouter>
            <LinkRouter to="/Accounts/payment-settings" className="block hover:underline">Configuración de pago</LinkRouter>
            <div className="pt-4">
              <LinkRouter to="/Accounts/settings" className="flex items-center hover:underline">
       ...
       <div className="flex-1 absolute top-0 right-0 w-3/4 p-4">
        <Outlet />
       </div>
      </div>
    </div>
    <UserProfileModal isOpen={isProfileModalOpen} onClose={() => setIsProfileModalOpen(false)} />
      <PurchaseHistoryModal  isOpen={isPurchaseHistoryModalOpen} onClose={() => setIsPurchaseHistoryModalOpen(false)} />
    </div>
  );
};
